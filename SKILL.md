...
description: >-
  ...
  ③指标对比查询(全量矩阵 73指标×235地区，按洲分区，公式透视+标准体系映射+来源URL可追溯)。
  农残/污染物逐物质展开，强制含微生物/致病菌指标。v1.9.0 升级为全量指标(5大类73项)×全量地区(235个)矩阵 +
  内置Codex/EU/US权威数据(BASE_LIMITS) + 标准体系映射(采纳国标注适用标准框架+URL)，[待填写]占比约29%（实测28.8%）。
...
def build_workbook(out_dir, food_name, basic, translations, risks, merge_mode=False):
    ...
    #   规模：73 指标 × 235 地区 = 17305 有效单元格（Sheet4 实际 85 行 × 243 列含洲分隔列）
    ...
```

> **v1.9.0 全量矩阵+权威数据设计要点**：
> - **全量指标**：5 大类 73 项（食品添加剂 18 + 农药残留 22 + 污染物 13 + 真菌毒素 7 + 微生物/致病菌 13）
...
> - **Sheet4**：73 指标 × 235 地区按洲分区矩阵（85 行 × 243 列），单元格经 `resolved_cell()` 解析：内置权威数据(精确值+URL) / 适用标准框架(Codex/EU等+URL) / 不适用 / `[待填写]`，约 29% 为 `[待填写]`
