...
description: >-
  ...
  工作簿结构(v1.12.0)：①基础信息(含配料多语对照) ②食品安全风险识别表(31列，含指标名称四语/法规名称三语/「声称执行标准」)
  ③指标对比查询(全量矩阵 126指标×235地区，按洲分区，公式透视+标准体系映射+来源URL可追溯)。
  农残/污染物逐物质展开，强制含微生物/致病菌、理化质量、感官指标。v1.12.0（跨 Agent 通用）在全量指标(7大类126项)×全量地区(235个)矩阵基础上 +
  内置Codex/EU/US/JP/CN五套权威数据(BASE_LIMITS, 133条) + 标准体系映射回填(采纳/参照Codex/EU/US/JP地区直接回填具体限量值，绿色数据+外链)，[待填写]占比降至约0.7%（实测0.69%，含用户risks.json时）。
...
> **跨 Agent 通用说明（v1.12.0 起）**：本技能**不绑定任何特定智能体平台**。核心是一份方法说明（`SKILL.md`）+ 一个纯 Python 生成器（`scripts/build_xlsx.py`，仅依赖 `openpyxl`），可在任意支持「技能 / Skill」机制的 AI 智能体（WorkBuddy、Claude Code、Cursor、OpenAI Codex、通用 LLM + 工具链等）中运行。文档中所有「WebSearch / WebFetch / 提问 / 子智能体 / 写文件」等表述，均指你**当前运行环境对应的能力**，请按所在平台替换。各平台加载与调用指引见 `README.md`「各智能体平台中使用」与根目录 `CHANGELOG.md`。
...
- **输入判别**：若用户输入命中「附录 A 类别」却是**品类而非具体产品**（如「饮料」「零食」），须向用户确认（用你运行环境的提问/交互机制）：①提供具体产品名；或 ②按该品类做扫描（明确标注「品类级扫描，非单品」）。
- **检索深度档位**（通过提问让用户选择，默认「增强」）：
...
- **输出目录**：默认当前工作目录下的 `food-safety-<食品名>/`（如 `food-safety-螺蛳粉/`）；可通过提问让用户指定其他路径。该目录即生成器 `out_dir`，其下自动生成 `<食品名>_食品安全风险识别表.xlsx` 与 `standards/`。

### Phase 1 · 基础信息录入引导
- 接收一个或多个食品名称（支持批量）。
- **类别归属**：通过提问在「附录 A」确认/选择；用户已给则回填。
  - **批量跳过模式**：若一次多个食品，先问一次「逐条确认 / 全部自动分类(跳过提问，按名称推断类别)」，选后者则不逐条弹问。
- **配料/成分补充**：提示填写完整配料表（可粘贴标签原文）。结构化字段：主料/辅料/食品添加剂（INS 编号或功能类别）/已知过敏原。
- 用户不知道时用你运行环境的联网检索工具检索「典型配料」作建议，但必须标「建议值，需用户确认」，**不得**当事实写入。
...
- 调用你运行环境的**联网检索 / 网页抓取工具**，对 Phase 3 名称集合检索；严格遵守**每辖区查询上限**。
- **去重键（关键）**：行级去重键 = `(辖区, 标准编号, 风险类型)`，**不得仅按辖区去重**——同一辖区（如 CN）内的 GB 2760/GB 2762/GB 2763 属不同标准，必须分别成行。「已覆盖辖区」集合仅用于进度播报（X/Y 辖区），不参与行级去重。
- **检索工具全失败降级**：若你运行环境的联网检索与网页抓取工具均不可用/失败，不得生成空表，应产出「人工核验清单」（列出本应检索的辖区+风险类型+建议官方入口），并在 Excel 备注与对话中显式提示「检索工具不可用，待人工核验」。
...
- **文件与网页存档（含降级，重点）**：
  ...
  - **网页存档**：你运行环境的网页抓取工具返回的是**模型摘要**，另存为 `<代码>_<标准号>_<风险类型缩写>_page.md`；若沙箱允许用 `curl -L` 抓取**原始 HTML**，存 `<代码>_<标准号>_<风险类型缩写>_page.html`（**注意：JS 渲染页面抓不全，亦可能被墙/拦截**）；路径记入 Excel「网页存档路径」；
  - **下载/抓取失败降级**：仅保留 URL + 抓取摘要（`.md`），「备注」标「下载受限/抓取受限」，**不得伪造文件**。
- **并行模式（可选，呼应大规模检索）**：当档位=全量或辖区数>20，将区域分簇（如 东亚/欧盟/美洲/中东非/大洋洲），用多个**子智能体**并行检索各簇（若你的运行环境支持并行子智能体 / 多 agent 调度；不支持则顺序检索）。
  - **子智能体上下文内联（必须）**：子智能体是独立上下文，不会继承本 skill；派发时须在提示词中**完整内联**附录 B 源清单、Sheet3 列结构（31 列）、置信度定义、去重键规则与文件命名规则，否则检索会脱靶。
  - 各子智能体回传其簇的风险行（含来源/置信度/去重键）；主 agent 做**跨簇去重（按去重键）+ 冲突仲裁 + 合并**，再进入 Phase 5。

### Phase 5 · 生成 Excel + 文件归档
- 使用 **Python（openpyxl）** 生成 `.xlsx`（需 Python 3.10+ 与 `pip install openpyxl`；若运行环境无 Python，可用等效的 Excel 生成能力替代）。生成器见附录八，已含空值保护与 `standards/` 目录自动创建。
...
- **存档诚实**：网页抓取摘要 ≠ 原网页；JS 页/被墙页抓不全；下载失败不伪造文件。
...
> v1.9.0 全量矩阵+权威数据参考实现：单品模式不输出「食品名称」列。`ALL_INDICATORS` 维护 7 大类 126 项全量指标，`ALL_REGIONS`(regions_data.py) 维护 235 个全量地区(按洲分组)。`BASE_LIMITS` 内置 Codex/EU/US 权威限量数据(每条带来源URL)；`CODEX_STD_BY_CATEGORY`/`EU_STD_BY_CATEGORY` 提供指标类别→国际标准框架映射；`resolved_cell()` 按优先级解析单元格：内置权威数据→risks.json用户数据→不适用→采纳国标准体系映射(标注适用Codex/EU标准框架+URL)→[待填写]。Sheet3 按全量清单遍历；Sheet4 为 126×235 按洲分区矩阵，绿/黄/红/白色标记数据/适用标准/不适用/待填写。

```python
# FoodSafeML v1.12.0 — 全量指标(7大类126项)×全量矩阵生成器（核心参考实现）
# 完整可运行版本见 scripts/build_xlsx.py + scripts/regions_data.py（235个地区）
# 核心设计：ALL_INDICATORS(7大类126项) × ALL_REGIONS(235个) + BASE_LIMITS(权威数据) + 标准体系映射
...
def build_workbook(out_dir, food_name, basic, translations, risks, merge_mode=False):
    ...
    #             （内置权威数据/适用标准框架+URL/不适用/[待填写]），约 0.7% 单元格为 [待填写]（v1.11.0 压实后；实测 0.69% 含用户数据）
...
> - **Sheet4**：126 指标 × 235 地区按洲分区矩阵（141 行 × 243 列），单元格经 `resolved_cell()` 解析：内置权威数据(精确值+URL) / 适用标准框架(Codex/EU等+URL) / 不适用(感官依产品标准评定) / `[待填写]`，约 0.7% 为 `[待填写]`（v1.11.0 压实后；实测 0.69%）
...
**现象**：TeamCreate 建立的团队会在**跨轮次会话间被自动回收**，连带把仍在跑的子智能体一起清掉，表现为"无声消失"——agent 永远不回传，文件也不落盘。
**对策**：
...
**对策**：
- 派发子智能体时，在提示词中**强制要求**：「完成后必须用你环境的文件写入能力把完整结果写入 `<输出目录>/<食品名>/<食品名>_<簇名>_检索结果.md`，并在回复中附小结」；
...
**现象**：不同子智能体写的表头列名可能带或不带括号后缀（如 `有效性(现行有效/已废止/…)` vs `有效性`），导致解析器按精确匹配时整表 0 行。
**对策**：
- 派发提示词中**固定表头列名**（不带括号后缀），列序 18 列严格一致；
- 整合脚本用**列序索引**而非列名匹配解析（按 `|` 分割后的列位置取值），对表头微差异容错。

### 10.4 后端网络 502 / ENOTFOUND 抖动
**现象**：`copilot.tencent.com` 偶发 502 ENOTFOUND，导致子智能体整体失败（非逻辑错误）。
...
- 两个脚本均用 Python 虚拟环境（如 venv）的 openpyxl，重跑只需两条命令：
  ```bash
...
### 10.14 v1.12.0 跨 Agent 通用·技能解耦（2026-07-12）

用户要求「这个技能不是只给 WorkBuddy 用，其它 agent 智能体也要通用」，本轮完成**平台解耦**与日志完善：

- **去除 WorkBuddy 私有耦合**：`SKILL.md` / `README.md` 中所有平台专属写法重写为能力化描述——
  - `~/.workbuddy/skills/` 路径 → 通用「运行输出目录 / 各平台 skills 目录」；
  - `WebSearch` / `WebFetch` → 「你运行环境的联网检索 / 网页抓取工具」；
  - `AskUserQuestion` → 「通过提问让用户确认（用你运行环境的提问/交互机制）」；
  - `Agent` 工具并行 → 「多个子智能体（若运行环境支持并行子智能体 / 多 agent 调度；不支持则顺序检索）」；
  - `Write` 工具 → 「你环境的文件写入能力」；`minimax-xlsx` 私有技能 → 「等效的 Excel 生成能力」。
  - 新增「跨 Agent 通用说明」与 README「各智能体平台中使用」小节，给出 WorkBuddy / Claude Code / Cursor / OpenAI Codex / 通用 LLM+工具链 的加载与调用指引。
- **确认生成器已跨平台**：`scripts/build_xlsx.py` 仅依赖 `os/re/json/sys/openpyxl`，走 `sys.argv` 传参，**无任何平台 SDK 耦合**（grep 验证）；在任意装有 Python 3.10+ 与 openpyxl 的环境可直接 `python build_xlsx.py <食品目录> [食品名]` 运行。
- **新增根目录 `CHANGELOG.md`**：作为跨平台可读的版本索引，覆盖 v1.7.0 → v1.12.0。
- **修正旧版附录八过期数字**：「5 大类 72 项」→「7 大类 126 项」；「约 27% / 29% 单元格 [待填写]」→「约 0.7%（实测 0.69%，含用户数据）」。
- 版本 1.11.0 → **1.12.0**；`SKILL_VER`、示例产物「skill 版本」单元格同步更新。
